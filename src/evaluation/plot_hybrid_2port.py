...
def plot_2port_learning_curve(dataset_type='link'):
    """Plots learning curve and causality ratio for 2-port hybrid."""
    history_path = os.path.join(PROJ_ROOT, f"results/checkpoints/training_history_hybrid_2port_{dataset_type}.json")
    
    if not os.path.exists(history_path):
        print(f"  [SKIP] No 2-port hybrid history found for {dataset_type.upper()}")
        return
    ...
    fig.suptitle(f'2-Port Hybrid RationalNet - {dataset_type.upper()} Dataset', fontsize=16, fontweight='bold')
    ...
    if 'causality_ratio' in history and history['causality_ratio']:
        ax2.plot(epochs, [r * 100 for r in history['causality_ratio']], color='green', linewidth=2)
        ax2.axhline(y=80, color='red', linestyle='--', alpha=0.7, label='80% threshold')
        ax2.set_title('Causality Ratio', fontsize=12)
    ...
    plt.tight_layout()
    save_path = os.path.join(PROJ_ROOT, f"results/learning_curve_hybrid_2port_{dataset_type}.png")
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"  [INFO] Learning curve saved to {save_path}")
    plt.close()

def plot_2port_s_parameters(dataset_type='link'):
    """Plots Sdd11 and Sdd21 with ground truth, total, and rational-only curves."""
    device = torch.device('cpu')
    
    checkpoint_path = os.path.join(PROJ_ROOT, f"results/checkpoints/best_hybrid_2port_{dataset_type}.pth")
    if not os.path.exists(checkpoint_path):
        print(f"  [SKIP] No 2-port checkpoint found for {dataset_type.upper()}")
        return
    ...
    if not os.path.exists(data_path) or not os.path.exists(poles_path):
        print(f"  [SKIP] Missing files for {dataset_type.upper()}")
        return
    ...
    num_ports = checkpoint.get('num_ports', 2)
    ...
    # Slice ground truth to 2x2 differential
    S_total_np = S_total[0].numpy()
    S_rational_np = S_rational[0].numpy()
    S_true_np = (y_r[0, :, :2, :2] + 1j * y_i[0, :, :2, :2]).numpy()
    ...
    # Plot 2x2 grid: Sdd11, Sdd12, Sdd21, Sdd22
    fig, axs = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(f'2-Port Hybrid - {dataset_type.upper()} (Causal: {causal_ratio:.1%})', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    save_path = os.path.join(PROJ_ROOT, f"results/s_params_2port_hybrid_{dataset_type}.png")
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    ...
    ax.set_title(f'Pole Map - {dataset_type.upper()} 2-Port Hybrid', fontsize=14, fontweight='bold')
    ax.grid(True, ls="--", alpha=0.6)
    
    save_path_poles = os.path.join(PROJ_ROOT, f"results/pole_map_2port_hybrid_{dataset_type}.png")
...
if __name__ == "__main__":
    ...
    for dtype in ['array', 'link']:
        checkpoint_path = os.path.join(PROJ_ROOT, f"results/checkpoints/best_hybrid_2port_{dtype}.pth")
        history_path = os.path.join(PROJ_ROOT, f"results/checkpoints/training_history_hybrid_2port_{dtype}.json")
        if os.path.exists(checkpoint_path) or os.path.exists(history_path):
            datasets_to_plot.append(dtype)
    
    if not datasets_to_plot:
        print("[ERROR] No 2-port hybrid checkpoints found. Run train_hybrid_2port.py first.")
    else:
        print(f"[INFO] Found 2-port data for: {', '.join(d.upper() for d in datasets_to_plot)}\n")
        for dtype in datasets_to_plot:
            print(f"{'='*60}")
            print(f"  Generating 2-port hybrid plots for {dtype.upper()}")
            print(f"{'='*60}")
            plot_2port_learning_curve(dataset_type=dtype)
            plot_2port_s_parameters(dataset_type=dtype)
            print()
        
        print("[SUCCESS] All 2-port hybrid plots generated.")