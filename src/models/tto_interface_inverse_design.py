"""
tto_interface_inverse.py
--------------------------------------------------
Test-Time Optimization (TTO) Inference Pipeline.

1. Uses the trained Tandem cVAE to generate an initial geometric guess.
2. Uses the Frozen Forward Surrogate + Element-Aware Weight Tensor to run
   gradient descent directly on the predicted physical geometry.
3. Plots Target vs cVAE Guess vs TTO Polished geometry S-parameters.
4. SAVES the generated design (geometry + target + prediction + topology) so
   the OpenEMS validation pipeline (stage 07) can build and verify it.
...
INVERSE_RUNS_DIR = PROJECT_ROOT / "src" / "models" / "inverse_runs"
...
OUT_DIR = PROJECT_ROOT / "results" / "models" / "evaluation_results" / "tto_inverse_design_interface"
...
def run_tto_inference(sample_idx=42, tto_steps=75, lr=0.05):
    ...
    forward_model = DirectSequenceResNet(d_local=xl_true.shape[1], d_global=xg.shape[1],
    ...
    # ---------------------------------------------------------------------
    # SAVE the generated design for OpenEMS validation (stage 07)
    # ---------------------------------------------------------------------
    design_dir = OUT_DIR / "generated_designs"
    design_dir.mkdir(parents=True, exist_ok=True)

    sim_ids = list(payload["sim_ids"])
    pair_ids_arr = (payload["pair_ids"].cpu().numpy() if hasattr(payload["pair_ids"], "numpy")
                    else np.asarray(payload["pair_ids"]))
    template_sim = str(sim_ids[sample_idx])
    pair_id = int(pair_ids_arr[sample_idx])

    design_path = design_dir / f"design_sample_{sample_idx}.npz"
    np.savez(
        design_path,
        x_local_norm=xl_opt.detach().cpu().numpy(),       # (1, 8) TTO-optimized geometry (normalized)
        x_local_cvae=xl_guess.detach().cpu().numpy(),     # (1, 8) cVAE initial guess
        x_local_true=xl_true.detach().cpu().numpy(),      # (1, 8) ground-truth geometry
        x_global_norm=xg.detach().cpu().numpy(),          # (1, 6)
        x_context_norm=xc.detach().cpu().numpy(),         # (1, 7)
        target_real=S_tgt.real.detach().cpu().numpy(),    # (1, 401, 4, 4) target S
        target_imag=S_tgt.imag.detach().cpu().numpy(),
        pred_real=S_tto.real.detach().cpu().numpy(),      # (1, 401, 4, 4) forward-model prediction
        pred_imag=S_tto.imag.detach().cpu().numpy(),
        template_sim_id=np.array([template_sim]),
        pair_id=np.array([pair_id]),
    )
    print(f"[stage-07 export] saved generated design -> {design_path}")
    print(f"    template sim: {template_sim}   pair: {pair_id}")
...
if __name__ == "__main__":
    for idx in [7063, 11742, 9557, 10151, 7633, 9754, 10227]:
        print("\n" + "=" * 60)
        print(f"  RUNNING SAMPLE {idx}")
        print("=" * 60)
        run_tto_inference(sample_idx=idx)