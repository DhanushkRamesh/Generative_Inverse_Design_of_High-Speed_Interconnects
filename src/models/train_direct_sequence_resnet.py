...
Includes Targeted Frequency Weighting for 112G Nyquist Optimization.
...
# Goldilocks Noise Floor: -45 dB perfectly isolates deep crosstalk nulls 
# without forcing the network to overfit to HFSS numerical mesh noise.
NOISE_FLOOR_DB = -45.0
...
#compute_element_weights: Computes per-element weights for the physics-informed loss.
...
class DirectSequenceResNet(nn.Module):
    ...
    Structurally guarantees S = S^T (Reciprocity).
    Includes Electromagnetic Positional Encoding (Phase/Length) for MTL Physics.
    """
    ...
    def __init__(self, d_local=8, d_global=6, d_context=7, F_len=401, hidden_dim=384, n_blocks=8):
        super().__init__()
        self.F_len = F_len
        self.is_link_dataset = (d_global == 7) # Link has 7 global features (includes LENGTH)
        ...
        # Learnable phase velocity scalar for the transmission line standing wave
        self.learnable_velocity = nn.Parameter(torch.tensor(10.0))
        
        # Input channels: hidden_dim (geometry) + 1 (freq) + 2 (sin/cos phase embeddings)
    ...
    # scatter_symmetric: Transforms (B, 10, F) back to symmetric (B, F, 4, 4) complex matrices.
    ...
    #forward: Main forward pass of the DirectSequenceResNet model.
    def forward(self, x_local, x_global, x_context, freqs_hz):
        ...
        # 3. Electromagnetic Phase Embeddings (Transmission Line Physics)
        if self.is_link_dataset:
            # LENGTH is the last feature in x_global for the Link dataset
            length_feature = x_global[:, -1].view(B, 1, 1)
        else:
            # Fallback for Array dataset (no horizontal length)
            length_feature = torch.zeros((B, 1, 1), device=x.device, dtype=h_seq.dtype)
            
        # Calculate standing wave phase: theta = velocity * f * L
        phase = self.learnable_velocity * f_norm * length_feature
        sin_emb = torch.sin(phase)
        cos_emb = torch.cos(phase)
        
        # 4. Process Sequence with explicit physics injected
        h = torch.cat([h_seq, f_norm, sin_emb, cos_emb], dim=1) # (B, hidden_dim + 3, F)
        h = self.proj_in(h)
...
def composite_loss(pred, target, weight44):
    """Direct Sequence Loss with 112G Frequency Weighting and Passivity Penalties"""
    eps = 1e-12
    F_len = pred.shape[1]
    
    # -------------------------------------------------------------------------
    # NEW: Targeted Frequency Weighting for 112G Nyquist Optimization
    # 401 points = 0 to 100 GHz. 60 GHz is approx index 240.
    # We apply a 5x penalty to errors in the critical 0-60 GHz passband.
    # -------------------------------------------------------------------------
    freq_weights = torch.ones(F_len, device=pred.device)
    freq_weights[:240] = 5.0  
    freq_weights[240:] = 1.0  
    # Linear Magnitude Loss (L1) with frequency weighting
    ...
    # Huber Magnitude Loss with strict noise floor
    ...
    # Total Loss Mapping (Passivity penalty increased to 10.0 for strict bounds)
...
def main():
    ...
    run_dir = RUNS_DIR / datetime.now().strftime(f"run_%d-%m-%y_%H%M%S_direct_resnet_{args.dataset.lower()}")
