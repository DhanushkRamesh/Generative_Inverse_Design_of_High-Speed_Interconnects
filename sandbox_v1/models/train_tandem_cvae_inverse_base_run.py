"""
tandem_cvae_inverse_base.py
--------------------------------------------------
Baseline Generative Inverse Design using a Conditional Variational Autoencoder (cVAE).

This script establishes the foundational Tandem architecture. It trains the 
cVAE to generate geometries that match target S-parameters using the frozen 
Forward Model as the physics evaluator. 

Note: Yield Optimization (Robustness/Jacobian Loss) has been explicitly removed 
to allow the network to map deep high-frequency resonant nulls without penalty.
Includes 'Save Best Model' tracking based on Physics S-parameter loss.
...
# =============================================================================
# FORWARD MODEL DEFINITION (Frozen Evaluator)
# =============================================================================
...
# =============================================================================
# INVERSE GENERATIVE ARCHITECTURE (BASE cVAE)
# =============================================================================
...
# =============================================================================
# BASELINE TRAINING LOOP (No Jacobian Loss)
# =============================================================================
def train_inverse_epoch_base(cvae, forward_model, loader, freqs_hz, device, optimizer):
    cvae.train()
    ...
    for xl, xg, xc, S_tgt in pbar:
        ...
        # 3. Baseline Physics Loss (S-Parameter Matching ONLY)
        ...
        # Composite Loss Aggregation (No Yield Penalty)
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Baseline Tandem cVAE Inverse Geometry Generator")
    ...
    forward_model = DirectSequenceResNet(
        ...
        d_context=payload["X_context"].shape[1], F_len=len(freqs_hz), hidden_dim=384, n_blocks=8
    ).to(device)
    ...
    # Initialize Generative Inverse Model
    cvae = Tandem_cVAE(
        d_local=payload["X_local"].shape[1], d_global=payload["X_global"].shape[1],
    ...
    run_dir = RUNS_DIR / datetime.now().strftime(f"run_%Y-%m-%d_%H%M%S_inverse_base_cvae_{args.dataset.lower()}")
    run_dir.mkdir(parents=True, exist_ok=True)

    print("\nInitiating Baseline Tandem Generative Training Cycle (No Yield Loss)...")
    
    best_physics_loss = float('inf')

    for epoch in range(1, args.epochs + 1):
        stats = train_inverse_epoch_base(cvae, forward_model, train_loader, freqs_hz, device, optimizer)
        
        # NEW: Best Model Tracking Logic
        marker = ""
        if stats['physics_s'] < best_physics_loss:
            best_physics_loss = stats['physics_s']
            torch.save(cvae.state_dict(), run_dir / "cvae_base_best_physics.pt")
        ...
        print(f"Epoch {epoch:03d}/{args.epochs} | "
              ...
              f"KLD: {stats['kld']:.4f}{marker}")
              
        if epoch % 20 == 0:
            torch.save(cvae.state_dict(), run_dir / f"cvae_base_checkpoint_ep{epoch}.pt")

    torch.save(cvae.state_dict(), run_dir / "cvae_base_final.pt")
