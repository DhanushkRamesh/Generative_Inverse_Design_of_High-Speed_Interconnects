...
def draw_progress_bar(epoch, total_epochs, metrics_str, bar_len=20):
    ...
    # Carriage return overwrites the line, creating a clean updating display
    sys.stdout.write(f"\rEpoch {epoch:03d}/{total_epochs} [{bar}] | {metrics_str}")
    sys.stdout.flush()
...
def main():
    ...
    sys.stdout.write("Starting Training Pipeline...\n")
    for epoch in range(1, total_epochs + 1):
        ...
        for x, y in train_loader:
            ...
            t_db = 20 * torch.log10(torch.clamp(torch.abs(y), min=1e-5))
            
            # RESEARCH FIX: Clamping the dB loss at the physical noise floor
            p_db_clamped = torch.clamp(p_db[:, :, :2, :2], min=NOISE_FLOOR_DB)
            t_db_clamped = torch.clamp(t_db[:, :, :2, :2], min=NOISE_FLOOR_DB)
            loss_db = nn.functional.huber_loss(p_db_clamped, t_db_clamped)
            
            loss = loss_cplx + (0.01 * loss_db) + (0.01 * loss_reg)
            loss.backward()
        ...
        with torch.no_grad():
            for xv, yv in val_loader:
                ...
                p_db_clamped = torch.clamp(p_db, min=NOISE_FLOOR_DB)
                t_db_clamped = torch.clamp(t_db, min=NOISE_FLOOR_DB)
                
                val_s11_err += torch.abs(p_db_clamped[:, :, 0, 0] - t_db_clamped[:, :, 0, 0]).mean().item()
                val_s21_err += torch.abs(p_db_clamped[:, :, 1, 0] - t_db_clamped[:, :, 1, 0]).mean().item()
        ...
        history_val_db.append(avg_db)
        
        scheduler.step(avg_db)
        
        marker = ""
        if avg_db < best_avg_mae:
            best_avg_mae = avg_db
            epochs_without_improvement = 0
        ...
        metrics_str = (
            ...
            f"TLin:{avg_train_lin:.3f} VLin:{avg_lin:.3f} "
    ...
    ax2 = ax1.twinx()
    ax2.plot(history_val_db, label='Val Avg dB MAE', color='red', linestyle='--')
    ax2.set_ylabel('Avg dB Error', color='red')
