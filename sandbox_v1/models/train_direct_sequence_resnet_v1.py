...
Incorporates the Symmetric Kramers-Kronig (Hilbert) Layer to guarantee Causality.
Enforces Reciprocity via scatter and Passivity via SVD penalty.
...
# Raised noise floor to ignore HFSS mesh noise artifacts
NOISE_FLOOR_DB = -40.0
...
class DirectSequenceResNet(nn.Module):
    """
    Maps Geometry -> 401-point Real Sequence directly via 1D Convolutions.
    Calculates Imaginary Sequence via Kramers-Kronig to guarantee Causality.
    """
    UPPER_R = (0, 0, 0, 0, 1, 1, 1, 2, 2, 3)
    UPPER_C = (0, 1, 2, 3, 1, 2, 3, 2, 3, 3)

    def __init__(self, d_local=8, d_global=6, d_context=7, F_len=401, hidden_dim=256, n_blocks=6):
        super().__init__()
        self.F_len = F_len
        ...
        # Output ONLY 10 channels for the Real part
        self.proj_out = nn.Conv1d(hidden_dim, 10, kernel_size=1) 
    ...
    def kramers_kronig_imag(self, x_real: torch.Tensor) -> torch.Tensor:
        """
        Symmetric FFT-based Discrete Hilbert Transform.
        Derives the Imaginary response from the Real response to enforce Causality.
        """
        F_len = x_real.shape[-1]
        
        # 1. Mirror Real part for Conjugate Symmetry (Nyquist to DC)
        x_mirror = torch.flip(x_real[..., 1:-1], dims=[-1])
        X_even = torch.cat([x_real, x_mirror], dim=-1)
        N = X_even.shape[-1]
        
        # 2. Time Domain (Impulse Response)
        h_time = torch.fft.ifft(X_even, dim=-1).real
        
        # 3. Causal Window
        causal_window = torch.zeros(N, device=x_real.device, dtype=x_real.dtype)
        causal_window[0] = 1.0
        causal_window[1 : F_len-1] = 2.0
        causal_window[F_len-1] = 1.0
        
        h_causal = h_time * causal_window
        
        # 4. Transform back and extract Imaginary
        X_causal = torch.fft.fft(h_causal, dim=-1)
        x_imag = X_causal.imag[..., :F_len]
    ...
    def forward(self, x_local, x_global, x_context, freqs_hz):
        ...
        # 2. Sequence Expansion + Freq Injection
        ...
        # Predict Real, Calculate Imaginary
        vec_real = self.proj_out(h) # (B, 10, F)
        vec_imag = self.kramers_kronig_imag(vec_real) # (B, 10, F)
...
def composite_loss(pred, target, weight44):
    ...
    # Passivity Penalty (Evaluate on 10 random frequencies per batch)
    F_len = pred.shape[1]
    f_idx = torch.randint(0, F_len, (10,), device=pred.device)
    sv = torch.linalg.svdvals(pred[:, f_idx, :, :])
    # Increased to 20.0 to strictly enforce max singular value <= 1.0
    l_passivity = F.relu(sv - 1.0).mean()

    total = 1.0 * l_linear + 0.1 * l_db + 0.5 * l_weighted + 20.0 * l_passivity
...
def main():
    ...
    train_idx, val_idx = splits["train_idx"].numpy(), splits["val_idx"].numpy()
    ...
    print(f"\nModel: Direct Sequence Kramers-Kronig ResNet ({sum(p.numel() for p in model.parameters()):,} parameters)")

    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=10, min_lr=1e-6)

    run_dir = RUNS_DIR / datetime.now().strftime("run_%Y-%m-%d_%H%M%S_direct_resnet_kk")
