"""
train_forward_v6.py
Training pipeline for the v6 parallel-decomposition rational forward model.
Tracks dual delta scales and enforces strict L2 weight decay.
"""
...
from rational_forward_v6 import RationalForwardModel  
...
def main():
    ...
    # INCREASED REGULARIZATION DEFAULT
    ...
    print(f"\nModel: rational_forward_v6, M={args.M} ({n_params:,} parameters)")
    ...
    run_id = datetime.now().strftime("run_%Y-%m-%d_%H%M%S_v6")
    if args.dry_run:
        run_id += "_dryrun"
    ...
    with open(log_path, "w", newline="") as f:
        header = ["epoch", "train_loss", "val_loss",
                  ...
                  "lr", "epoch_time_s"]
        for i in range(args.M):
            header.append(f"delta_re_sub{i}")
            header.append(f"delta_im_sub{i}")
    ...
    history = {"epoch": [], "train_loss": [], "val_loss": [],
               "val_sdd11": [], "val_sdd21": [],
               "delta_scales_re": [], "delta_scales_im": []}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        deltas_re, deltas_im = model.get_delta_scales()
        ...
        d_im_min, d_im_max = min(deltas_im), max(deltas_im)
        metrics_str = (
            ...
            f" avg:{avg_db:5.2f}dB d_im:[{d_im_min:.2f}-{d_im_max:.2f}]"
        ...
        history["delta_scales_re"].append(deltas_re)
        history["delta_scales_im"].append(deltas_im)
        ...
        row = [epoch, train_stats["loss"], val_stats["loss"],
               ...
               cur_lr, dt]
        for dr, di in zip(deltas_re, deltas_im):
            row.extend([dr, di])
    ...
    deltas_im_arr = np.array(history["delta_scales_im"]) 
    for i in range(args.M):
        axes[2].plot(history["epoch"], deltas_im_arr[:, i], label=f"sub {i} (freq)")
    axes[2].set_xlabel("epoch"); axes[2].set_ylabel("delta_scale_im per sub-model")
    ...
    print(f"  Final delta_scales_im: {[f'{d:.2f}' for d in history['delta_scales_im'][-1]]}")
