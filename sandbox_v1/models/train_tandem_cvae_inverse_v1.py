"""
tandem_cvae_inverse_weighted.py
--------------------------------------------------
Generative Inverse Design using a cVAE with High-Frequency Weighted Loss.

This script implements Novel Upgrade 1: The S-parameter physics loss is 
dynamically weighted. Errors at high frequencies (where complex resonances occur)
are penalized up to 5x more heavily than DC/low-frequency errors.
...
# =============================================================================
def train_inverse_epoch_weighted(cvae, forward_model, loader, freqs_hz, device, optimizer):
    cvae.train()
    acc = {"loss": 0.0, "kld": 0.0, "recon_x": 0.0, "physics_s": 0.0, "n": 0}
    pbar = tqdm(loader, desc="Training Batches", leave=False)
    
    # Pre-calculate Frequency Weights (Scale: 1.0 at DC up to 5.0 at 100 GHz)
    f_norm = freqs_hz / freqs_hz.max()
    f_weights = (1.0 + f_norm * 4.0).view(1, -1, 1, 1).to(device)
    for xl, xg, xc, S_tgt in pbar:
        ...
        # Apply the High-Frequency Penalty
...
def main():
    ...
    run_dir = RUNS_DIR / datetime.now().strftime(f"run_%Y-%m-%d_%H%M%S_inverse_weighted_{args.dataset.lower()}")
    run_dir.mkdir(parents=True, exist_ok=True)
    best_physics = float('inf')

    print("\nInitiating Frequency-Weighted Tandem Generative Training...")
    for epoch in range(1, args.epochs + 1):
        stats = train_inverse_epoch_weighted(cvae, forward_model, train_loader, freqs_hz, device, optimizer)
        marker = " <-- [NEW BEST]" if stats['physics_s'] < best_physics else ""
        if marker: 
            best_physics = stats['physics_s']
            torch.save(cvae.state_dict(), run_dir / "cvae_weighted_best.pt")
