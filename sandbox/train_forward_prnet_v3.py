...
# ==========================================
# 2. Augmented PRNet (15 Poles, Stable Bounds)
# ==========================================
class AugmentedPRNet(nn.Module):
    ...
    def forward(self, x, s_tensor, freqs_ghz):
        ...
        # Stable Physics Bounds (alpha limited to -0.05 to prevent infinite spikes)
        ...
        # Out-of-place assembly to protect gradients
        H_s11 = H_s[:, 0, :] 
...
# ==========================================
def main():
    results_dir = "forward_augmented_final_results"
    ...
    # The most stable, proven SI loss function
    def stable_loss(pred, target):
        ...
        # SmoothL1 is highly stable. It won't cause gradient starvation.
        db_smooth = nn.functional.smooth_l1_loss(p_db, t_db)
        
        return mse_c + (0.05 * db_smooth)

    print(f"Training Final Augmented PRNet on {device}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_v_db < best_val_mae:
            ...
            torch.save(model.state_dict(), os.path.join(results_dir, "best_augmented_prnet.pth"))

        if (epoch + 1) % 50 == 0 or epoch == 0:
            print(f"Epoch {epoch+1:03d} | Loss: {avg_loss:.6f} | Val MAE: {avg_v_db:.2f} dB | Best: {best_val_mae:.2f} dB")

    # ==========================================
    # 4. Final Verification (Using BEST weights)
    # ==========================================
    model.load_state_dict(torch.load(os.path.join(results_dir, "best_augmented_prnet.pth")))
    model.eval()
    ...
    plt.plot(ds.freqs_ghz, p_db, 'r--', label='Augmented PRNet (15 Poles)', linewidth=2)
    plt.title(f"Final Augmented Verification (Best MAE: {best_val_mae:.2f} dB)")
    plt.ylabel("Magnitude (dB)"); plt.xlabel("Frequency (GHz)")
    plt.grid(True, alpha=0.3); plt.legend()
    
    save_path = os.path.join(results_dir, "augmented_final_proof.png")
