...
def composite_loss(pred, target, weight44):
    ...
    # The network learns magnitude and phase smoothly without phase-wrapping shocks
...
class ProgressBar:
    ...
    def finish(self, final_line: str = ""):
        sys.stdout.write("\n")
        if final_line: sys.stdout.write(final_line + "\n")
        sys.stdout.flush()
...
def main():
    ...
    run_dir = RUNS_DIR / datetime.now().strftime("run_%Y-%m-%d_%H%M%S_direct_resnet")
