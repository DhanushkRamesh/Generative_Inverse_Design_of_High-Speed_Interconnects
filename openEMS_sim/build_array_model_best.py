...
class ArrayModelBuilder:
    ...
    def run_and_extract(self, sim_root: Path, run: bool = True):
        ...
        for p_exc in range(N):
            ...
            FDTD.SetBoundaryCond(["PML_8", "PML_8", "PML_8", "PML_8", zbc, zbc])
...
def main():
    ...
    _logfile = open(_THIS_DIR / f"stage05_best_log_{datetime.now():%Y%m%d_%H%M%S}.log", "w")
    ...
    if args.dry_run:
        ...
        np.savez(out / f"{args.sim}_best_openems_se.npz", freq=freq, S=S)
        print(f"\nSaved (50-ohm referenced, de-embedded to board surface):")
        print(f"  {out / (args.sim + '_best_openems_se.npz')}")
