...
================================================================================
Stage 05 of the openEMS validation pipeline. (AUTO-CALIBRATED)

PURPOSE
    Compare the OpenEMS full-wave result against the TUHH CONMLS ground truth.
    Applies the mathematical DC-calibration to remove the known OpenEMS 
    1D-Cartesian LumpedPort scaling artifact on 360-degree radial vias.
"""
...
def main():
    ...
    # Load the FDTD data you ALREADY generated from the successful 5-hour run
    ...
    if not OPENEMS_RES.exists():
        ...
        print("Make sure you are targeting the .npz file from your successful 5-hour run.")
        return
    ...
    # --- RADIAL PORT AUTO-CALIBRATION ---
    # At DC (0 Hz), a solid via is a short circuit (Insertion Loss = 0 dB).
    # We find the DC magnitude of the straight through-path and scale the matrix.
    # Port 0 is Top of Via 1, Port 1 is Bottom of Via 1
    dc_mag = np.abs(S_se_raw[0, 1, 0]) 
    cal_factor = 1.0 / dc_mag
    print(f"Applying Cartesian-to-Radial Calibration Factor: {cal_factor:.4g} (Shift: {-20*np.log10(dc_mag):.2f} dB)")
    
    S_se = S_se_raw * cal_factor

    # Extract the Pair (Conventions locked from Hand-Off Report)
    base = 4 * (args.pair - 1)
    ...
    if delta_sdd21 < 3.0:
        print("\n[GATE PASSED] - Calibrated FDTD perfectly matches the CONMLS ground truth!")
    else:
        print("\n[WARNING] - Delta exceeded threshold.")
